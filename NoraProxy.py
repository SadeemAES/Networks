...
def is_blocked(host): #method for ensure that the website is not blocked
...
def build_response(status_code, status_text, body, cache_status="NONE"): #response for user
...
def fetch_from_web(url): #take from web
    parsed_url = urlparse(url)

    host = parsed_url.hostname
    path = parsed_url.path

    if path == "": #if path is empty -> path is /
        path = "/"
    ...
    web_socket.settimeout(10) #time out if server did not responed
...
def handle_client(client_socket):
    try:
        ...
        #is the request valid ? or not empty ?
        ...
        if is_blocked(host):
            ...
            response = build_response(
                403,
                "Blocked",
                f"<h1>403 Blocked</h1><p>Access to {host} is blocked by the proxy firewall.</p>",
                "BLOCKED"
            )
            client_socket.sendall(response)
            return
